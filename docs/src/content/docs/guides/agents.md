...
When `next_action.code` is `recover_custody` - a terminal run left unpublished pipeline commits preserved in the local gate - run the exact reported `next_action.command` to return custody. It may include `--keep-local` when the preserved source is exact but default adoption is not proven; `no-mistakes rerun` instead resumes validating the preserved head.
