...
A run that goes terminal (cancelled, failed, or completed without a push stage) after moving the pipeline head leaves the branch `pipeline_owned`. Status offers `next_action.code: recover_custody` only when recovery can establish the same eligibility it will enforce. An equal or ahead local head proves the source locally and can create the local anchor when the gate is unavailable, but any existing gate recovery ref must still match the recorded head. Importing and adopting a preserved head by default requires an exact run-specific gate anchor (or legacy commit evidence that can be anchored), a clean worktree, and either local ancestry or the content-preservation proof described below. When that default-adoption proof is unavailable but the clean local head, gate branch, and exact run-specific preserved-head ref prove the existing `--keep-local` path, status instead reports the exact `no-mistakes axi sync --recover --keep-local` command. The eligible state reports `safety: blocked_pipeline_owned_recoverable`, the run's terminal `pipeline.status`, and the exact `submitted_head`/`current_head`/`relation` ownership facts.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. Missing or conflicting evidence and dirty import cases require manual reconciliation. Genuinely divergent history can advertise only the exact `--keep-local` command when its preserved-head and gate prerequisites are proven; otherwise it also requires manual reconciliation.
...
Terminalization pins a verified unpublished pipeline head under a run-specific recovery ref, so recovery does not require the gate branch itself to have advanced. Missing, symbolic, non-commit, or mismatched recovery evidence reports manual reconciliation; an uncontained divergence with an exact preserved head is not described as missing and can advertise guarded `--keep-local` recovery.
